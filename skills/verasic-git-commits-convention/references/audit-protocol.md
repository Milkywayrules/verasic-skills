...
| Invoke                                    | Output                                                                                                  |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `/verasic-git-commits-audit --help`           | Help only — no git audit                                                                                |
| `/verasic-git-commits-audit`                  | Full audit report (default scope below)                                                                 |
| `/verasic-git-commits-audit --unpushed-only`  | Limit range to `@{upstream}..HEAD` (fail clearly if no upstream)                                        |
| `/verasic-git-commits-audit --base develop`   | Use merge-base with `origin/develop` instead of the detected default branch                             |
| `/verasic-git-commits-audit --author "Name"`  | Override author filter (default: `git config user.name`)                                                |
| `/verasic-git-commits-audit --include-merges` | Include merge commits in message scan (still skip for file scan unless noted)                           |
| `/verasic-git-commits-audit --fix-trailers`   | **Rewrite history** to strip co-authored trailers — only when user explicitly approves after audit      |

Flags compose: `/verasic-git-commits-audit --unpushed-only --base master`
...
packs, rules, and subagents wired into the repo) are allowed.

| Category | Blocker | OK when intentional |
| -------- | ------- | ------------------- |
| Agent config install | — | `.cursor/skills/**`, `.cursor/rules/**`, `.cursor/agents/**`, `.agents/skills/**` committed as repo tooling (e.g. installing/updating Verasic skill packs) |
