# Scanner adapter — verasic-secbot
...
```bash
bash .cursor/skills/verasic-secbot/scripts/run-scanner.sh <off|opengrep|semgrep|auto> -- "${CHANGED_FILES[@]}"
```

(or `.agents/skills/verasic-secbot/scripts/run-scanner.sh` for cursor-hybrid installs)
...
```bash
bash .cursor/skills/verasic-secbot/scripts/run-scanner.sh semgrep -- "${CHANGED_FILES[@]}"
```
...
```bash
bash .cursor/skills/verasic-secbot/scripts/run-scanner.sh <off|opengrep|semgrep|auto> -- file1 file2
```
