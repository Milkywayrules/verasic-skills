...
If no security-relevant surface detected, the orchestrator may add ONE line: `Tip: no security surface in this diff — run /verasic-bugbot for general bug hunting.` Never more than one line, never a finding, never auto-chain.

## Artifacts

When `securityReview.report.write` is true (default), write markdown copies of the full report.

On first report write, ensure artifact dirs exist — no config file scaffold:

```bash
mkdir -p verasic/security-reviews .verasic/security-reviews
```
