...
setup_repo() {
  ...
  cp -r "$SKILL_ROOT" "$dir/.cursor/skills/verasic-github-cli-init"
  cd "$dir"
  git init -q -b main
...
bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh >/dev/null
...
assert_fail "check-gh rejects fake token" "bash .cursor/skills/verasic-github-cli-init/scripts/check-gh.sh"
assert_stderr_contains \
...
assert_fail "check-gh missing token" "bash .cursor/skills/verasic-github-cli-init/scripts/check-gh.sh"
...
assert \
  ...
  "bash -c 'set +eu; cd sub; source \"../.cursor/skills/verasic-github-cli-init/scripts/load-gh-env.sh\"; [[ \"\$-\" != *e* && \"\$-\" != *u* ]] && [[ \"\$(pwd)\" == */sub ]] && [[ -n \"\$GH_TOKEN\" ]] && ! type _verasic_extract_var >/dev/null 2>&1'"
assert \
  "load-gh-env preset GH_TOKEN wins" \
  "bash -c 'set +eu; export GH_TOKEN=preset; source .cursor/skills/verasic-github-cli-init/scripts/load-gh-env.sh; [[ \"\$GH_TOKEN\" == preset ]]'"
...
rc=0
bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh >/dev/null 2>&1 || rc=$?
if [[ "$rc" -eq 3 ]]; then
...
warn_out="$(bash .cursor/skills/verasic-github-cli-init/scripts/check-gh.sh 2>&1 >/dev/null || true)"
...
chmod 644 .github-agent.local
bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh >/dev/null 2>&1 || true
...
printf '%s\n' 'GH_TOKEN=' 'GH_REPO=acme/x' > .github-agent.local
chmod 644 .github-agent.local
bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh --no-chmod >/dev/null 2>&1 || true
mode="$(stat -c '%a' .github-agent.local 2>/dev/null || stat -f '%Lp' .github-agent.local)"
if [[ "$mode" == 644 ]]; then
...
out="$(bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh 2>&1 || true)"
...
chmod 600 .github-agent.local
rc=0
out="$(bash .cursor/skills/verasic-github-cli-init/scripts/bootstrap.sh 2>&1)" || rc=$?
